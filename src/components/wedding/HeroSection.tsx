...
const HeroSection = () => {
  ...
  const targetDate = new Date(content.wedding_date);
  const now = new Date();
  ...
  const groomInitial = content.groom_name.charAt(0);
  const brideInitial = content.bride_name.charAt(0);
      ...
      <section id="about" className="py-24 md:py-32">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="space-y-20">
            <div className="grid grid-cols-2 gap-6 md:gap-10 max-w-2xl mx-auto">
              <div className="flex flex-col items-center gap-4">
                <div className="relative">
                  <img src={content.child_groom_url} alt={`${content.groom_name} в детстве`} className="w-full aspect-square object-cover rounded-lg shadow-md" />
                  <div className="absolute -bottom-3 -right-3 w-14 h-14 border-2 rounded-lg" style={{ borderColor: "hsl(var(--wedding-gold))" }} />
                </div>
                <p className="font-serif text-lg text-muted-foreground italic">{content.groom_name}</p>
              </div>
              <div className="flex flex-col items-center gap-4">
                <div className="relative">
                  <img src={content.child_bride_url} alt={`${content.bride_name} в детстве`} className="w-full aspect-square object-cover rounded-lg shadow-md" />
                  <div className="absolute -bottom-3 -left-3 w-14 h-14 border-2 rounded-lg" style={{ borderColor: "hsl(var(--wedding-gold))" }} />
                </div>
                <p className="font-serif text-lg text-muted-foreground italic">{content.bride_name}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 md:gap-10 max-w-2xl mx-auto">
              <div className="flex flex-col items-center gap-4">
                <img src={content.adult_groom_url} alt={content.groom_name} className="w-full aspect-square object-cover rounded-lg shadow-md" />
                <p className="font-serif text-lg text-muted-foreground italic">Выросли</p>
              </div>
              <div className="flex flex-col items-center gap-4">
                <img src={content.adult_bride_url} alt={content.bride_name} className="w-full aspect-square object-cover rounded-lg shadow-md" />
            ...
            <div className="max-w-md mx-auto">
              <div className="relative">
                <img src={content.couple_image_url} alt={`${content.groom_name} и ${content.bride_name}`} className="w-full aspect-[3/4] object-cover rounded-lg shadow-lg" />
